...
void rc5_cts_encrypt16(data16 ptext, data16 ctext, uint16_t *ksched, size_t rounds, uint16_t *IV)
{
	...
	*(ptext.text + 1) ^= *(IV + 1);
	...
	uint16_t *temp[2];
	*temp[0] = *(ctext.text + ptext.len - 2);
	*temp[1] = *(ctext.text + ptext.len - 1);
	*(ctext.text + ptext.len - 2) = *(ctext.text + ptext.len - 4);
	*(ctext.text + ptext.len - 1) = *(ctext.text + ptext.len - 3);
	*(ctext.text + ptext.len - 4) = *temp[0];
	*(ctext.text + ptext.len - 3) = *temp[1];
	*temp[0] = 0;
	*temp[1] = 0;
}
