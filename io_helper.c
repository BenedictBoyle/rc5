...
{
	/* Take input buffer, return pointer to list of blocks PKCS #7 padded to appropriate length for ECB mode
	 */

	size_t pad = ((size_t) 4) - (input.inlen % 4);
	indata p_input;
...
data32 prepare_data32(indata input)
{
	...
	size_t pad = ((size_t) 8) - (input.inlen % 8);
	indata p_input;
	p_input.inlen = input.inlen + pad;
	p_input.inbuf = realloc(input.inbuf, p_input.inlen);
	if (p_input.inbuf == NULL) {
...
data64 prepare_data64(indata input)
{
	...
	size_t pad = ((size_t) 16) - (input.inlen % 16);
	indata p_input;
	p_input.inlen = input.inlen + pad;
	p_input.inbuf = realloc(input.inbuf, p_input.inlen);
	if (p_input.inbuf == NULL) {
